...
```bash
# Google Search
opencli google search "OpenAI" --limit 20

# Collect deeper result sets automatically
opencli google search "OpenAI" --limit 100 -f json
...
- `google search --limit` now pages through Google results in 10-result batches and supports up to 100 results.
- Deep searches can still stop early if Google stops rendering more organic results or shows an anti-bot interstitial. In that case, OpenCLI returns the results collected so far and prints a warning.
